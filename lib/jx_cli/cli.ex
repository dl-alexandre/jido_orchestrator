defmodule JX.CLI do
  ...
  defp dispatch(["runtimes", "provision", action_id | args]) do
    {opts, rest, invalid} =
      OptionParser.parse(args,
        strict: [
          project: :string,
    ...
    with :ok <- validate_options(invalid),
         ...
         :ok <- validate_required_option("project", opts[:project]),
         :ok <- validate_optional_positive("concurrency-limit", opts[:concurrency_limit]),
         :ok <- validate_optional_positive("ttl-seconds", opts[:ttl_seconds]),
         :ok <- start_app(),
         {:ok, runtime} <-
           Workspace.provision_runtime_for_action(action_id,
             project: opts[:project],
             host: opts[:host],
             runner_id: opts[:runner],
             tools: Keyword.get_values(opts, :tool),
             capabilities: Keyword.get_values(opts, :capability),
             os: opts[:os],
             branch_isolation: opts[:branch_isolation] || "worktree",
             concurrency_limit: opts[:concurrency_limit] || 1,
             ttl_seconds: opts[:ttl_seconds] || 24 * 60 * 60
           ) do
      print_runtime("provisioned", runtime, json: opts[:json] || false)
      :ok
    end
  end

  defp dispatch(["runtimes", "assign", runtime_id, action_id | args]) do
    {opts, rest, invalid} =
      OptionParser.parse(args,
        strict: [
          runner: :string,
    ...
    with :ok <- validate_options(invalid),
         ...
         :ok <- start_app(),
         {:ok, result} <-
           Workspace.assign_runtime_action(runtime_id, action_id,
             runner_id: opts[:runner],
             session_id: opts[:session],
             ttl_seconds: opts[:ttl_seconds] || 24 * 60 * 60
           ) do
      print_runtime_assignment(result, json: opts[:json] || false)
      :ok
    end
  end

  defp dispatch(["runtimes", "release", runtime_id | args]) do
    {opts, rest, invalid} = OptionParser.parse(args, strict: [json: :boolean])

    with :ok <- validate_options(invalid),
         :ok <- expect_no_args(rest, "jx runtimes release <runtime-id> [--json]"),
         :ok <- start_app(),
         {:ok, runtime} <- Workspace.release_runtime(runtime_id) do
      print_runtime("released", runtime, json: opts[:json] || false)
      :ok
    end
  end

  defp dispatch(["runtimes", "show", runtime_id | args]) do
    {opts, rest, invalid} = OptionParser.parse(args, strict: [json: :boolean])

    with :ok <- validate_options(invalid),
         :ok <- expect_no_args(rest, "jx runtimes show <runtime-id> [--json]"),
         :ok <- start_app() do
      case Workspace.get_runtime_environment(runtime_id) do
        nil ->
          {:error, :runtime_not_found}

        runtime ->
          print_runtime("runtime", runtime, json: opts[:json] || false)
          :ok
      end
    end
  end

  defp dispatch(["runtimes", "ls" | args]) do
    {opts, rest, invalid} =
      OptionParser.parse(args,
        strict: [
          status: :string,
    ...
    with :ok <- validate_options(invalid),
         ...
         :ok <- validate_positive("n", limit),
         :ok <- start_app() do
      Workspace.list_runtime_environments(
        status: opts[:status] || "active",
        workspace_id: opts[:workspace],
        runner_id: opts[:runner],
        limit: limit
      )
      |> print_runtimes(json: opts[:json] || false)
  ...
  defp dispatch(["approvals", "ls" | args]) do
    ...
    with :ok <- validate_options(invalid),
         :ok <-
           expect_no_args(
             rest,
             "jx approvals ls [--status open|acknowledged|dismissed|active|all] [--source devide] [--workspace <id>] [--kind <kind>] [-n 50] [--json]"
           ),
         :ok <- validate_optional_approval_status(opts[:status]),
         :ok <- validate_optional_approval_source(opts[:source]),
         :ok <- validate_optional_approval_kind(opts[:kind]),
         :ok <- validate_positive("n", limit),
         :ok <- start_app() do
      Workspace.list_approvals(
        status: opts[:status],
        source: opts[:source],
        workspace_id: opts[:workspace],
        kind: opts[:kind],
      ...
      |> print_approvals(json: opts[:json] || false)

      :ok
    end
  end

  defp dispatch(["approvals", "show", approval_id | args]) do
    {opts, rest, invalid} = OptionParser.parse(args, strict: [json: :boolean])

    with :ok <- validate_options(invalid),
         :ok <- expect_no_args(rest, "jx approvals show <id> [--json]"),
         :ok <- start_app(),
         {:ok, detail} <- Workspace.approval_detail(approval_id) do
      print_approval_detail(detail, json: opts[:json] || false)
      :ok
    end
  end

  defp dispatch(["approvals", "ack", approval_id | args]) do
    {opts, rest, invalid} = OptionParser.parse(args, strict: [json: :boolean])

    with :ok <- validate_options(invalid),
         :ok <- expect_no_args(rest, "jx approvals ack <id> [--json]"),
         :ok <- start_app(),
         {:ok, approval} <- Workspace.acknowledge_approval(approval_id) do
      print_approval_status("acknowledged", approval, json: opts[:json] || false)
      :ok
    end
  end

  defp dispatch(["approvals", "dismiss", approval_id | args]) do
    {opts, rest, invalid} = OptionParser.parse(args, strict: [json: :boolean])

    with :ok <- validate_options(invalid),
         :ok <- expect_no_args(rest, "jx approvals dismiss <id> [--json]"),
         :ok <- start_app(),
         {:ok, approval} <- Workspace.dismiss_approval(approval_id) do
      print_approval_status("dismissed", approval, json: opts[:json] || false)
      :ok
    end
  end

  defp dispatch(["approvals" | _args]) do
    {:error,
     "usage: jx approvals ls [--status open|acknowledged|dismissed|active|all] [--source devide] [--workspace <id>] [--kind proposal_conflict|unsafe_db|failed_run|policy_blocked] [-n 50] [--json] | jx approvals show <id> [--json] | jx approvals ack <id> [--json] | jx approvals dismiss <id> [--json]"}
  end
  ...
  defp validate_optional_approval_status(status) do
    statuses = JX.Approvals.statuses()
  ...
  defp runtimes_provision_usage do
    "jx runtimes provision <action-id> --project <project> [--host <host>] [--runner <runner-id>] [--tool <tool>] [--capability <cap>] [--os <os>] [--branch-isolation worktree] [--concurrency-limit 1] [--ttl-seconds 86400] [--json]"
  end

  defp runtimes_assign_usage do
    "jx runtimes assign <runtime-id> <action-id> [--runner <runner-id>] [--session <session-id>] [--ttl-seconds 86400] [--json]"
  end

  defp runtimes_ls_usage do
    "jx runtimes ls [--status planned|provisioning|ready|assigned|released|failed|expired|active|all] [--workspace <id>] [--runner <id>] [-n 50] [--json]"
  end

  defp runtimes_usage do
  ...
  defp help_group_usages do
    %{
      ...
      "runtimes" => [
        ...
        "jx runtimes show <runtime-id> [--json]",
        "jx runtimes release <runtime-id> [--json]",
        "",
