...
if [ "$#" -lt 1 ]; then
  printf '%s\n' "usage: scripts/meet_chat_bridge_watch.sh <session-id> [extra jx watch args...]" >&2
  exit 64
fi
...
CHAT_INPUT="${JX_MEET_CHAT_INPUT:-$ROOT/tmp/meet-chat-input.txt}"
...
mkdir -p "$(dirname "$CHAT_INPUT")"
touch "$CHAT_INPUT"

exec "$JX_BIN" meet realtime watch "$SESSION_ID" \
  --chat-file "$CHAT_INPUT" \
